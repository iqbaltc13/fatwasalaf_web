        ...
        <div class="uk-width-auto@s">
            <div id="sc-dt-buttons">
                <div class="dt-buttons">
                    ...
                    <a class="dt-button buttons-copy buttons-html5 sc-button" href="{{route('dashboard.master.pembayaran.create')}}" ><span data-uk-icon="icon: plus"></span><span>Tambah Mutasi Pembayaran</span></a>
        ...
        <table width="100%" id="sc-dt-buttons-table" class="uk-table uk-table-striped dt-responsive datatable">
            ...
            <tbody>
               @foreach ($datas as $data)
                   <tr>
                       <td>{{$data->id}}</td>
                       <td>{{($data->nasabah)?$data->nasabah->nama:""}}</td>
                       <td>{{$data->catatan_verifikator}}</td>
                       <td>{{($data->verified_at != null)?'Diverifikasi':'Belum Diverifikasi'}}</td>
                       <td>{{date('d F Y H:i:s',strtotime($data->created_at))}}</td>
                       {{-- <td>
                            <div><a class="sc-button sc-button-primary sc-js-button-wave-light" href="{{route('dashboard.master.pembayaran.edit',$data->id)}}"><span data-uk-icon="icon: pencil"></span></span> Edit</a></div>
...
<script>
    ...
    //         let linkDetail = "{{route('dashboard.master.pembayaran.detail-json', ':id')}}";
    ...
    var vue_component = new Vue({
            ...
            methods:{
                ...
                deleteShow:function(id){
                    ...
                    let linkDelete = "{{route('dashboard.master.pembayaran.destroy', ':id')}}";
                ...
                deleteSubmit:function(){
                    ...
                    let route  =  "{{route('dashboard.master.pembayaran.destroy', ':id')}}";
